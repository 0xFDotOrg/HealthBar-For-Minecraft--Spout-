    ...
    public void setTitle(Player pl) {
        ...
        if (health > 0 && health <= maxHealth) {
            ...
            if (!HealthBar.healthTracker.isEmpty() && HealthBar.healthTracker.containsKey(pl)) {
            	healthChange = HealthBar.healthTracker.get(pl) - pl.getHealth();
            }
            if(healthChange > 0) {
            	lostHealth = new String(new char[(int) (healthChange * getDouble("Scale.barScale"))]).replace("\0", getString("Characters.barCharacter"));
            }
            if (health > 0) {
                goodHealth = new String(new char[(int) ((health) * getDouble("Scale.barScale"))]).replace("\0", getString("Characters.barCharacter"));
            }
            if (health < maxHealth) {
                hurtHealth = new String(new char[(int) ((maxHealth - health - (healthChange > 0 ? healthChange : 0)) * getDouble("Scale.barScale"))]).replace("\0", getString("Characters.barCharacter"));
            }
            String playerHealthBar = "§e§c§e\n" + "§" + getString("Colors.containerColor") + getString("Characters.startCharacter")
            		...
            		+ "§" + getString("Colors.hurtHealthColor") + hurtHealth 
			...
			HealthBar.healthTracker.put(pl, pl.getHealth());
        }
    }
